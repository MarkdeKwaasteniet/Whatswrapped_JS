...
// var EmojiData = require(['require', 'emoji-data']);
// var moment = require(['require', 'node_modules/moment/moment']);

// define(['require', 'emoji-data'], function (require) {
//     var EmojiData = require('emoji-data');
// });
// require.config({
//     baseUrl: '',
//     paths: { 
//         EmojiData: '../node_modules/emoji-data',
  ...
  for (let line of lines) {
    if (line[0] === "[") {
        if (line.split("]", 2)[1].includes(":") == true) {
            const date = moment(line.split("]", 1)[0].replace("[", ""), "DD/MM/YYYY, HH:mm:ss");
...
const data = [];
  ...
  filteredData = filteredData.filter(
    (entry) =>
      entry.date >= new Date("2022-01-01") &&
      entry.date <= new Date("2022-12-30")
  );

    const emojiRegex = new RegExp(EmojiData.map(regexEscape).join("|"), "g");
    filteredData.forEach(entry => {
        entry["Emojis"] = entry["Message"].match(emojiRegex) || [];
        entry["Sentence"] = entry["Message"].replace(emojiRegex, "");
    });
...
// totalFile.push(line);
// if (line[0] === "[") {
// const dateTime = new Date(line.split("]", 1)[0].replace("[", ""));
// dateTimeList.push(dateTime);
// const name = line.split("]", 1)[1].split(":", 1)[0];
// nameList.push(name);
// if (":" in line.split("]", 1)[1]) {
// const text = line.split("]", 1)[1].split(":", 1)[1].replace("\n", "");
// textList.push(text);
// } else {
// textList.push("-");
// }
